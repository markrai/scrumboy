...
Lane colors and sprint chips use `styles.css` CSS variables. Sprints, tags, and search filter board scope via `sprintId` / `tag` / `search` query params on `GET /api/board/{slug}` (and lane pagination); `board-filters.ts` only manages chips, URL params, and reload — filtering itself is server-side in the store. On durable projects, the `tag` filter resolves through the same canonical-name grouping as chip listings (`TagGroupKey`). Agile field labels and native `title` hover hints (`field-tooltips.ts`) localize with the active locale.
