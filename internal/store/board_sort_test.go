...
func TestGetBoard_SortOrder(t *testing.T) {
	...
	t.Run("newest first orders by created_at desc", func(t *testing.T) {
		_, _, _, cols, err := st.GetBoard(ctxOwner, &pc, []string{""}, "", AssigneeFilter{}, PriorityFilter{}, SprintFilter{Mode: "none"}, SortOrderNewest)
		if err != nil {
			t.Fatalf("GetBoard: %v", err)
		}
	...
	t.Run("oldest first orders by created_at asc", func(t *testing.T) {
		_, _, _, cols, err := st.GetBoard(ctxOwner, &pc, []string{""}, "", AssigneeFilter{}, PriorityFilter{}, SprintFilter{Mode: "none"}, SortOrderOldest)
		if err != nil {
			t.Fatalf("GetBoard: %v", err)
		}
	...
	t.Run("paged fast path matches sort order", func(t *testing.T) {
		_, _, _, cols, _, err := st.GetBoardPaged(ctxOwner, &pc, []string{""}, "", AssigneeFilter{}, PriorityFilter{}, SprintFilter{Mode: "none"}, SortOrderNewest, 10)
		if err != nil {
...
func TestListTodosForBoardLane_SortOrderPagination(t *testing.T) {
	...
	drainAll := func(t *testing.T, sortOrder SortOrder, pageSize int) []string {
		...
		for i := 0; i < 10; i++ { // hard cap so a pagination bug can't infinite-loop the test
			items, nextCursor, hasMore, err := st.ListTodosForBoardLane(ctxOwner, p.ID, DefaultColumnBacklog, pageSize, afterA, afterB, []string{""}, "", AssigneeFilter{}, PriorityFilter{}, SprintFilter{Mode: "none"}, sortOrder)
			if err != nil {
