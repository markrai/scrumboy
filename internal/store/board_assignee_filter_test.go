...
func TestGetBoard_AssigneeFilter(t *testing.T) {
	...
	t.Run("filters by specific assignee", func(t *testing.T) {
		...
		_, _, _, cols, err := st.GetBoard(ctxOwner, &pc, []string{""}, "", filter, PriorityFilter{}, SprintFilter{Mode: "none"}, SortOrderDefault)
		if err != nil {
	...
	t.Run("paged path combines assignee tag and search", func(t *testing.T) {
		filter := mustAssigneeFilter(t, strconv.FormatInt(u1.ID, 10), nil)
		_, _, _, cols, meta, err := st.GetBoardPaged(ctxOwner, &pc, []string{"focus"}, "u1", filter, PriorityFilter{}, SprintFilter{Mode: "none"}, SortOrderDefault, 1)
		if err != nil {
	...
	t.Run("paged path filters unassigned", func(t *testing.T) {
		filter := mustAssigneeFilter(t, "unassigned", nil)
		_, _, _, cols, meta, err := st.GetBoardPaged(ctxOwner, &pc, []string{""}, "", filter, PriorityFilter{}, SprintFilter{Mode: "none"}, SortOrderDefault, 1)
		if err != nil {
			t.Fatalf("GetBoardPaged: %v", err)
		}
		todos := cols[DefaultColumnBacklog]
...
func TestListTodosForBoardLane_AssigneeFilter(t *testing.T) {
	...
	filter := mustAssigneeFilter(t, strconv.FormatInt(u1.ID, 10), nil)
	items, cursor, hasMore, err := st.ListTodosForBoardLane(ctxOwner, p.ID, DefaultColumnBacklog, 2, math.MinInt64, 0, []string{""}, "", filter, PriorityFilter{}, SprintFilter{Mode: "none"}, SortOrderDefault)
	if err != nil {
	...
	items, _, hasMore, err = st.ListTodosForBoardLane(ctxOwner, p.ID, DefaultColumnBacklog, 2, afterRank, afterID, []string{""}, "", filter, PriorityFilter{}, SprintFilter{Mode: "none"}, SortOrderDefault)
	if err != nil {
	...
	count, err := st.CountTodosForBoardLane(ctxOwner, p.ID, DefaultColumnBacklog, []string{""}, "", filter, PriorityFilter{}, SprintFilter{Mode: "none"})
	if err != nil {
	...
	count, err = st.CountTodosForBoardLane(ctxOwner, p.ID, DefaultColumnBacklog, []string{""}, "", unassignedFilter, PriorityFilter{}, SprintFilter{Mode: "none"})
	if err != nil {
...
VALUES (?, ?, ?, '', ?, ?, NULL, ?, NULL, ?, ?, NULL)`)
	...
	_, _, _, cols, meta, err := st.GetBoardPaged(ctxOwner, &pc, []string{""}, "", filter, PriorityFilter{}, SprintFilter{Mode: "none"}, SortOrderDefault, 2)
	if err != nil {
		t.Fatalf("GetBoardPaged: %v", err)
	}
	...
	nextItems, _, _, err := st.ListTodosForBoardLane(ctxOwner, p.ID, DefaultColumnBacklog, 3, afterRank, afterID, []string{""}, "", filter, PriorityFilter{}, SprintFilter{Mode: "none"}, SortOrderDefault)
	if err != nil {
...
func TestGetBoardPaged_AssigneeSprintTagSearchComposition(t *testing.T) {
	...
	_, _, _, cols, meta, err := st.GetBoardPaged(ctxOwner, &pc, []string{"focus"}, "needle", filter, PriorityFilter{}, sprintFilter, SortOrderDefault, 10)
	if err != nil {
		t.Fatalf("GetBoardPaged: %v", err)
	}
	items := cols[DefaultColumnBacklog]
