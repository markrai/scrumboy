...
func (a *Adapter) validateMoveAnchors(ctx context.Context, projectID int64, columnKey string, afterTodo, beforeTodo *store.Todo, mode store.Mode) *adapterError {
	...
	if afterTodo != nil {
		items, _, _, err := a.store.ListTodosForBoardLane(ctx, projectID, columnKey, 1, afterTodo.Rank, afterTodo.ID, "", "", store.AssigneeFilter{}, store.SprintFilter{})
		if err != nil {
	...
	if beforeTodo != nil {
		...
		items, _, _, err := a.store.ListTodosForBoardLane(ctx, projectID, columnKey, 1, laneStartRank, 0, "", "", store.AssigneeFilter{}, store.SprintFilter{})
		if err != nil {
			return mapStoreError(err)
		}
