...
// refreshNotifyDebounce coalesces board.refresh_needed activity emails per
// (project, category): a burst of mutations (e.g. reordering many cards)
// would otherwise fan out one email per member per event. Only the first
// event in a window triggers a send; later ones in the same window are
...
type emailNotifier struct {
	...
	mu       sync.Mutex
	lastSent map[string]time.Time // key: "<projectID>:<category>"
}

func newEmailNotifier(st storeAPI, mq *mailQueue, publicBaseURL string, smtpConfigured bool, logger *log.Logger) *emailNotifier {
...
// shouldDebounce reports whether an activity notification for this
// project+category was already sent within refreshNotifyDebounce, and
// records this attempt as the most recent one.
func (n *emailNotifier) shouldDebounce(projectID int64, category emailCategory) bool {
	key := fmt.Sprintf("%d:%s", projectID, category)
	now := time.Now()
	n.mu.Lock()
	defer n.mu.Unlock()
...
func (n *emailNotifier) handleRefreshNeeded(ctx context.Context, e eventbus.Event) {
	...
	category, ok := refreshReasonCategory[p.Reason]
	if !ok {
		return
	}
	if p.ActorUserID == 0 {
	...
	if n.shouldDebounce(e.ProjectID, category) {
		return // coalesce a burst of same-category mutations into one email round
	}
	...
	members, err := n.store.ListProjectMembers(ctx, e.ProjectID, p.ActorUserID)
	if err != nil {
		return
	}
	...
	for _, m := range members {
		...
		n.send(m.Email, subject, body, fmt.Sprintf("email-notify category=%s user=%d", category, m.UserID))
	}
}
...
func (n *emailNotifier) send(to, subject, body, logRef string) {
