...
func TestEmailNotifier_TodoCreatorNotified_SendsWhenOptedIn(t *testing.T) {
	st := newTestStore(t)
	ctx := context.Background()
	n, mq := newTestEmailNotifier(t, st, true)

	creator, err := st.BootstrapUser(ctx, "creator@example.com", "pass1234A!", "Creator")
	if err != nil {
		t.Fatal(err)
	}
	actor, err := st.CreateUser(ctx, "actor@example.com", "pass1234A!", "Actor")
	if err != nil {
		t.Fatal(err)
	}
	creatorCtx := store.WithUserID(ctx, creator.ID)
	proj, err := st.CreateProject(creatorCtx, "Test Project")
	if err != nil {
		t.Fatal(err)
	}

	// Default prefs have "createdByMe" on, so no explicit opt-in call needed.
	payload, _ := json.Marshal(eventbus.TodoCreatorNotifiedPayload{
		ProjectID: proj.ID, TodoID: 1, Title: "Fix the bug", ActorUserID: actor.ID, CreatedByUID: creator.ID,
	})

	// Master toggle defaults to disabled; confirm no email until enabled.
	n.OnEvent(ctx, eventbus.Event{Type: "todo.creator_notified", ProjectID: proj.ID, Payload: payload})
	if got := drainAfterAsync(mq); len(got) != 0 {
		t.Fatalf("expected no email before master opt-in, got %+v", got)
	}
	...
	pref.Enabled = true
	prefJSON, _ := json.Marshal(pref)
	if err := st.SetUserPreference(ctx, creator.ID, "emailNotifications", string(prefJSON)); err != nil {
		t.Fatal(err)
	}
	...
	if got[0].To != "creator@example.com" {
		t.Fatalf("expected email to creator, got %+v", got[0])
	}
}

func TestEmailNotifier_TodoCreatorNotified_SkipsWhenOptedOut(t *testing.T) {
	st := newTestStore(t)
	ctx := context.Background()
	n, mq := newTestEmailNotifier(t, st, true)

	creator, err := st.BootstrapUser(ctx, "creator2@example.com", "pass1234A!", "Creator")
	if err != nil {
		t.Fatal(err)
	}
	actor, err := st.CreateUser(ctx, "actor2@example.com", "pass1234A!", "Actor")
	if err != nil {
		t.Fatal(err)
	}
	creatorCtx := store.WithUserID(ctx, creator.ID)
	proj, err := st.CreateProject(creatorCtx, "Test Project 2")
	if err != nil {
		t.Fatal(err)
	}
	...
	n.OnEvent(ctx, eventbus.Event{Type: "todo.creator_notified", ProjectID: proj.ID, Payload: payload})
	if got := drainAfterAsync(mq); len(got) != 0 {
		t.Fatalf("expected no email when createdByMe opted out, got %+v", got)
	}
}
