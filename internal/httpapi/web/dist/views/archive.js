...
import { getAuthStatusAvailable, getBoard, getBoardMembers, getEditingTodo, getProjectId, getSlug, getTagColors, getUser, } from '../state/selectors.js';
...
let archiveTodos = [];
let archiveNextCursor = null;
let archiveHasMore = false;
let archiveLoading = false;
let archiveLoadFailed = false;
let archiveSelectedLocalIds = new Set();
...
function installArchiveTagCatalog(catalog) {
    ...
    for (const tag of catalog) {
        ...
        for (const existingName of Object.keys(colors)) {
            if (existingName.toLocaleLowerCase() === key)
                delete colors[existingName];
        }
...
function archiveAssigneeHtml(todo) {
...
function archivePriorityHtml(todo) {
    if (!todo.priorityKey)
        return '';
...
function syncArchiveSelectionUi() {
    ...
    if (bar)
        bar.hidden = !canMutateArchive() || count === 0;
    if (countEl)
        countEl.textContent = t('archive.selectionCount', { count });
    if (restoreBtn)
        restoreBtn.disabled = count === 0 || count > ARCHIVE_BATCH_MAX || archiveLoading;
    document.querySelectorAll('[data-archive-local-id]').forEach((row) => {
...
function renderArchiveList() {
    ...
    const loadMore = document.getElementById('archiveLoadMoreBtn');
    const retry = document.getElementById('archiveRetryBtn');
    const selectAll = document.getElementById('archiveSelectAllBtn');
    if (!list || !status)
        return;
...
function archiveTopbarHtml(board) {
...
function renderArchiveShell(board) {
    app.innerHTML = `
  ...
  `;
    ...
    document.getElementById('archiveSelectAllBtn')?.addEventListener('click', () => {
        ...
        if (archiveTodos.length > ARCHIVE_BATCH_MAX)
    ...
    document.getElementById('archiveList')?.addEventListener('change', (event) => {
        ...
        if (checkbox.checked)
            archiveSelectedLocalIds.add(localId);
        else
            archiveSelectedLocalIds.delete(localId);
        syncArchiveSelectionUi();
    });
}
async function loadArchivePage(reset) {
    ...
    try {
        ...
        if (getSlug() !== slug)
            return;
        const byId = new Map();
        for (const todo of reset ? [] : archiveTodos)
        ...
        if (archiveTodos.length > 0)
            showToast(apiErrorMessage(error, { fallbackKey: 'archive.loadFailed' }));
    }
    finally {
...
async function restoreArchiveSelection() {
    ...
    if (!slug || !canMutateArchive() || localIds.length === 0)
        return;
...
async function openArchivedTodo(slug, localId) {
    try {
        const todo = await apiFetch(`/api/board/${encodeURIComponent(slug)}/todos/${localId}`);
        if (!todo.archivedAt) {
...
async function reconcileOpenArchivedTodo() {
    const slug = getSlug();
    const editing = getEditingTodo();
    if (!slug || !editing?.archivedAt || !window.location.pathname.includes('/archive/t/'))
        return;
    try {
        const latest = await apiFetch(`/api/board/${encodeURIComponent(slug)}/todos/${editing.localId}`);
...
function onArchiveRealtimeEvent(payload) {
    if (!archiveEventsSlug || getSlug() !== archiveEventsSlug)
        return;
    const event = payload;
    if (event.type === 'ping' || event.type === 'todo.assigned' || event.type === 'todo.creator_activity' || event.type === 'wall.refresh_needed' || event.type === 'wall.transient')
        return;
    const projectId = getProjectId();
    if (typeof event.projectId === 'number' && projectId != null && event.projectId !== projectId)
        return;
...
export function stopArchiveEvents() {
...
window.addEventListener('scrumboy:archive-list-changed', (event) => {
    ...
    if (!archiveEventsSlug || detail.slug !== archiveEventsSlug)
        return;
    scheduleArchiveRefresh();
});
export async function renderArchive(slug, openTodoSegment = null) {
    if (!slug)
    ...
    const [board, catalogResponse] = await Promise.all([
        apiFetch(`/api/board/${encodedSlug}?limitPerLane=1`),
        apiFetch(`/api/board/${encodedSlug}/tags`).catch(() => []),
    ]);
    if (sequence !== archiveRenderSequence)
        return;
    installArchiveTagCatalog(Array.isArray(catalogResponse) ? catalogResponse : []);
    ...
    if (!rendered || sequence !== archiveRenderSequence)
        return;
    ...
    if (openTodoSegment) {
        ...
        if (Number.isInteger(localId) && localId > 0)
            await openArchivedTodo(slug, localId);
    }
    else {
