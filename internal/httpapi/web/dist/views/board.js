    ...
    if (mobileTabsEl) {
        ...
        mobileTabsEl.querySelectorAll(".mobile-tab").forEach((tab) => {
            ...
            if (tabKey && tabLabels[tabKey]) {
                const count = getLaneDisplayCount(tabKey);
...
function renderBoardFromData(board, projectId, tag, search, sprintId, opts = {}) {
    ...
    <div class="page">
      ...
      <div class="container">
        ...
        return `
            <button class="mobile-tab ${getMobileTab() === c.key ? "mobile-tab--active" : ""}" data-tab="${c.key}"${tabStyle}><span class="mobile-tab__text">${escapeHTML(c.title)} ${getLaneDisplayCount(c.key)}</span></button>
            `;
    }).join("")}
        ...
        return boardCols
            ...
            return `
                <section class="col ${isMobileActive ? "col--mobile-active" : ""}" data-column="${c.key}">
                  <div class="col__head col__head--${c.key.toLowerCase()}" ${c.color ? `style="background:${escapeHTML(c.color)};"` : ""}>
                    <span class="col__title">${escapeHTML(c.title)}</span>
                    <span class="col__count" data-count-for="${c.key}">${getLaneDisplayCount(c.key)}</span>
                  </div>
                  <div class="col__list" data-status="${c.key}" id="list_${c.key}">
                    ${todos.map((t) => renderTodoCard(t, c.color, membersByUserId, cardOpts)).join("")}
                  </div>
