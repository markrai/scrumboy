 ...
 */
export function sanitizePostAuthNext(raw) {
    ...
    if (!decodedPath)
        return "/";
    const hasRejectedChar = (s) => {
        for (let i = 0; i < s.length; i++) {
            const code = s.charCodeAt(i);
            if (code === 0 || code === 0x0a || code === 0x0d || s[i] === "\\")
                return true;
        }
        return false;
    };
    if (hasRejectedChar(trimmed) || hasRejectedChar(decodedPath))
        return "/";
    ...
    if (decodedPath.includes("://"))
        return "/";
    if (trimmed.includes("#") || decodedPath.includes("#"))
        return "/";
