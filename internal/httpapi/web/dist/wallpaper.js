...
let pendingWallpaperAcquisition = null;
let ownedWallpaperResource = null;
function defaultOff() {
    return { v: 1, mode: 'off' };
}
...
function supersedeWallpaperAcquisition() {
...
function releaseOwnedWallpaperResource() {
    const owned = ownedWallpaperResource;
    ownedWallpaperResource = null;
    if (owned)
        releaseResource(owned.resource);
}
function clearServerWallpaperRendering(imgEl) {
...
function applyBuiltinWallpaper(shell, imgEl, generation) {
...
function installServerWallpaperResource(shell, imgEl, path, resource) {
    document.documentElement.setAttribute('data-wallpaper-active', '');
    document.documentElement.setAttribute('data-wallpaper-source', 'image');
    shell.classList.add('wallpaper-shell--visible');
    imgEl.style.backgroundColor = '';
    ...
    if (previous && previous.resource !== resource)
        releaseResource(previous.resource);
}
function acquireServerWallpaper(shell, imgEl, path) {
    if (pendingWallpaperAcquisition?.path === path)
        return;
...
export function applyWallpaperState(st) {
    ...
    if (st.mode === 'image' && st.rev) {
        acquireServerWallpaper(shell, imgEl, `/api/user/wallpaper/image?rev=${encodeURIComponent(String(st.rev))}`);
        return;
    }
    applyWallpaperState(defaultOff());
}
