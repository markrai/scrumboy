...
function slotsFor(targetRaw, laneRaw) {
...
function parseMoveSyntax(utterance) {
    ...
    const partitions = [...rest.matchAll(/\s+(?:to|as|into)\s+/gi)]
        ...
        return slots.lane ? [{ slots, separatorStart: separator.index }] : [];
    });
...
function laneExists(lane, board) {
    return !!lane && !!board && !isCommandFailure(resolveVoiceLane(lane, board));
}
function exactTitleExists(reference, board) {
    if (!reference || !board)
        return false;
...
function selectMovePartition(partitions, board) {
...
function operationCues(rest) {
    return [...rest.matchAll(SECOND_OPERATION)];
}
function partitionHasCompoundCue(parsed, partition, board) {
    const cues = operationCues(parsed.rest);
    if (!partition) {
        if (!cues.length && !POSSIBLE_CONTINUATION.test(parsed.rest))
            return false;
...
export function extractMoveSlotCandidates(utterance) {
    const parsed = parseMoveSyntax(utterance);
    if (!parsed)
        return [];
    if (parsed.partitions.length)
        return parsed.partitions.map(partition => partition.slots);
...
export function hasCompoundRequestCue(goal, board) {
    const parsed = parseMoveSyntax(goal);
    if (!parsed)
        return operationCues(goal).length > 0;
...
function moveClarification(slots) {
...
export function recognizeMoveCommand(utterance, board) {
    const parsed = parseMoveSyntax(utterance);
    if (!parsed)
        return null;
    const partition = selectMovePartition(parsed.partitions, board);
    if (parsed.partitions.length && !partition)
        return null;
    if (partitionHasCompoundCue(parsed, partition, board))
        return null;
...
export function extractDeterministicMoveCall(utterance, board) {
    const recognized = recognizeMoveCommand(utterance, board)?.envelope;
    return recognized?.kind === 'skill_call' ? recognized : null;
}
export function extractMissingMoveSlotClarification(utterance, board) {
    const recognized = recognizeMoveCommand(utterance, board)?.envelope;
