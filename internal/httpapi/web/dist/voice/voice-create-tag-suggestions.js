...
function isSingleInsertion(shorter, longer) {
    let left = 0;
    let right = 0;
    let skipped = false;
...
export function isVoiceCreateTagDistanceOne(left, right) {
    if (left === right || Math.abs(left.length - right.length) > 1)
        return false;
    if (left.length + 1 === right.length)
        return isSingleInsertion(left, right);
    if (right.length + 1 === left.length)
        return isSingleInsertion(right, left);
    const differences = [];
...
export function findVoiceCreateTagSuggestions(reference, authoritativeTags) {
    ...
    const terminal = terminalTarget
        ...
        : [];
    if (terminal.length)
        return Object.freeze({ matches: Object.freeze(terminal), kind: 'terminal_s' });
