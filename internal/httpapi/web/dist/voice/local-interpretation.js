    ...
    switch (intent.kind) {
        case 'create-todo': {
            ...
            if (intent.title === null)
                return { kind: 'create-todo', title: null };
        ...
        case 'move-todo': {
            ...
            if (intent.destination !== null) {
                if (!hasExactKeys(intent.destination, ['kind', 'text']))
                    rejectOutput();
                const candidate = intent.destination;
...
function transcriptContainsDialogueLocalId(transcript, localId) {
    return new RegExp(`(?:^|\\D)${String(localId)}(?:\\D|$)`).test(transcript);
}
function validateDialogueChoiceSelector(value, transcript, allowed) {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        rejectOutput();
    ...
    if (selector.kind === 'lane') {
        if (allowed !== 'todo' || !hasExactKeys(value, ['kind', 'text']))
            rejectOutput();
...
function validateProvideSlotDialogueIntent(value, transcript, pending) {
    if (!hasExactKeys(value, ['kind', 'operation', 'slot', 'value']))
        rejectOutput();
    if (value.operation !== pending.operation || value.slot !== pending.slot)
        rejectOutput();
    switch (pending.operation) {
        case 'todo.create':
        case 'todo.update_title': {
            if (pending.slot !== 'title')
                rejectOutput();
...
function validateCorrectionDialogueIntent(value, transcript, operation) {
    if (value.kind === 'correct-choice') {
        if (!hasExactKeys(value, ['kind', 'selector']))
            rejectOutput();
...
function validateDialogueIntent(value, transcript, context) {
    const pending = context.pending;
    if (!pending || !value || typeof value !== 'object' || Array.isArray(value))
        rejectOutput();
    const intent = value;
    ...
    if (pending.kind === 'todo-choice' || pending.kind === 'member-choice' || pending.kind === 'tag-choice') {
        if (intent.kind !== 'select-choice' || !hasExactKeys(value, ['kind', 'selector']))
            rejectOutput();
        return {
            kind: 'select-choice',
            selector: validateDialogueChoiceSelector(intent.selector, transcript, pending.kind === 'todo-choice' ? 'todo' : pending.kind === 'member-choice' ? 'member' : 'tag'),
        };
    }
    if (pending.kind === 'missing-slot') {
        if (intent.kind !== 'provide-slot')
            rejectOutput();
...
function validateTurnIntent(value, transcript, context) {
    if (value === null)
        return null;
    if (context?.pending)
        return validateDialogueIntent(value, transcript, context);
