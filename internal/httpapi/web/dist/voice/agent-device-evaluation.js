...
import { createVoiceAgentEvaluationModel, evaluateVoiceAgentCorpus, evaluateVoiceAgentUtterance, voiceAgentEvaluationCaseApplicability, VOICE_AGENT_EVALUATION_BOARD_REQUIREMENTS, VOICE_AGENT_EVALUATION_TIMEOUT_MS, } from './agent-evaluation.js';
function currentBoardContext() {
    return getVoiceCreateDryRunBoardPorts().getContext();
}
function emptyUtterance(transcript) {
...
async function evaluateOnCurrentBoard(transcript, options = {}) {
    ...
    if (!context)
        return emptyUtterance(transcript);
...
async function evaluateCorpusOnCurrentBoard(options = {}) {
    const capability = getAppRuntime().capability(LOCAL_TEXT_GENERATION_CAPABILITY);
    const context = currentBoardContext();
    const controller = new AbortController();
