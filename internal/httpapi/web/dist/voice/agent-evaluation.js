import { interpretAgentEnvelope } from './agent-protocol.js';
import { createVoiceAgentModel } from './agent-model.js';
import { extractHighConfidenceMoveCall, extractMissingMoveSlotClarification, recoverMoveAfterParseFailure, recoverPresentMoveArguments, extractMoveSlots, uniqueDirectMoveTitle, } from './agent-move-extraction.js';
...
export const VOICE_AGENT_EVALUATION_VERSION = 1;
export const VOICE_AGENT_EVALUATION_TIMEOUT_MS = 45000;
...
function hasLocalId(board, localId) {
    return boardTodos(board).some(todo => todo.localId === localId);
}
function hasLane(board, lane) {
    return !isCommandFailure(resolveVoiceLane(lane, board));
}
function ambiguousTitle(phrase, board) {
    if (uniqueDirectMoveTitle(phrase, board))
        return false;
 ...
 */
export function parseUnrecoveredModelEnvelope(raw) {
...
export function voiceAgentEvaluationCaseApplicability(testCase, board) {
...
const SCORE_MISS = Object.freeze({ matches: false, mismatches: Object.freeze(['kind']) });
export function scoreVoiceAgentEnvelope(envelope, expected, board) {
    try {
        if (!envelope)
            return { matches: false, mismatches: ['kind'] };
        ...
        if (expected.kind === 'skill_call' && kind !== 'skill_call')
        ...
        if (kind === 'skill_call' && skill === 'todos.move') {
            ...
            if (expected.referenceNumeric != null && numericFrom(reference) !== expected.referenceNumeric)
                mismatches.push('reference');
            if (expected.referenceText) {
                const normalized = (unwrapTodoReference(reference ?? '') ?? reference ?? '').toLowerCase();
                if (!normalized.includes(expected.referenceText.toLowerCase()))
                    mismatches.push('reference');
            }
            if (expected.lane && laneKey(stringField(args, 'lane'), board) !== laneKey(expected.lane, board))
                mismatches.push('lane');
        }
        if (kind === 'clarify_skill' && skill === 'todos.move') {
            if (expected.missing && stringField(record, 'missing') !== expected.missing)
...
function scoreEvaluationLayers(rawEnvelope, protocolEnvelope, guardedEnvelope, expected, board) {
    try {
        return {
...
export async function evaluateVoiceAgentUtterance(transcript, options) {
    ...
    let protocolRecoveredFrom = null;
...
export function createVoiceAgentEvaluationModel(capability, locale = 'en') {
    return createVoiceAgentModel(capability, locale);
}
export async function evaluateVoiceAgentCorpus(cases, options) {
    const results = [];
