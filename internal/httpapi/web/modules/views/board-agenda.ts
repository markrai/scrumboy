...
function agendaHostKind(raw: string | undefined): 'google' | 'apple' | 'other' {
  if (raw === 'google' || raw === 'apple') return raw;
  return 'other';
}

export function agendaSourceChips(events: AgendaEvent[]): AgendaSourceChip[] {
  const chips: AgendaSourceChip[] = [];
...
export function agendaMobileTabAriaLabel(board: Board): string {
...
export function agendaMobileTabInnerHtml(eventCount: number): string {
  return `${AGENDA_CALENDAR_DAYS_SVG} <span class="mobile-tab__count">${escapeHTML(String(eventCount))}</span>`;
}

export function agendaLaneColor(board: Board): string {
...
function agendaHostLabel(chip: AgendaSourceChip): string {
  const named = typeof chip.calendarName === 'string' ? chip.calendarName.trim() : '';
  if (named) return named;
  const kind = agendaHostKind(chip.hostKind);
  if (kind === 'google') return t('board.agenda.badge.google');
  if (kind === 'apple') return t('board.agenda.badge.apple');
  return t('board.agenda.badge.other');
}

function renderAgendaHeaderHost(chip: AgendaSourceChip): string {
  const kind = agendaHostKind(chip.hostKind);
...
export function renderAgendaHeaderHosts(events: AgendaEvent[]): string {
...
export function agendaNowContextTopMinute(nowMinute: number, viewportMinutes: number): number {
  return nowMinute - viewportMinutes * AGENDA_SMART_NOW_OFFSET_FRACTION;
}

export type AgendaSmartFocusInput = {
...
export function agendaSmartFocusMinute(input: AgendaSmartFocusInput): number {
  ...
  const canUseNow =
    input.isToday && nowMinute != null && Number.isFinite(nowMinute) && viewportMinutes > 0;
  if (!canUseNow) return baseline;
...
function applyInitialFocusToList(list: HTMLElement, board: Board, now?: Date): boolean {
  if (!agendaListIsLaidOut(list)) return false;
  ...
  const baseline = agendaInitialFocusMinute(
    events,
    timezone,
    agendaStartOfDayMinutes(),
    clock,
...
export function flushAgendaInitialScroll(): void {
  ...
  if (!applyInitialFocusToList(list, board, pendingInitialFocus.now)) return;
  clearPendingInitialFocus();
}
...
export function syncOpenBoardAgendaLayout(opts: { forceAutoFocus?: boolean } = {}): void {
