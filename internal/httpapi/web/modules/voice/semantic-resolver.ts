...
export type VoiceSemanticSelection = VoiceConversationSelection;

export type VoiceSemanticResolution =
  | Readonly<{
      kind: 'command';
  ...
  | Readonly<{
      ...
      selection: VoiceSemanticSelection;
      interaction: Extract<VoiceSemanticInteraction, { kind: 'clarification' }>;
    }>
  | Readonly<{
      kind: 'information';
      target?: VoiceTodoReference;
      interaction: Extract<VoiceSemanticInteraction, { kind: 'information' }>;
    }>;

export type VoiceSemanticResolveContext = Readonly<{
  projectId: number;
  projectSlug: string;
  board: Board;
...
type ResolvedTodoCandidate = Readonly<{
  todo: Todo;
  reference: VoiceTodoReference;
  lane: VoiceLaneReference;
}>;

export type VoiceSemanticCandidateDisposition =
  | 'actionable'
  | 'already-satisfied'
  | 'ineligible';

type ClassifiedTodoCandidate = Readonly<{
  candidate: ResolvedTodoCandidate;
  disposition: VoiceSemanticCandidateDisposition;
}>;

type MembersListResponse = Readonly<{
  items?: BoardMember[];
}>;

type TodoGetResponse = Readonly<{
  todo?: Todo;
}>;

type CompletedTodoCountResponse = Readonly<{
  count?: number;
}>;

function todoReference(context: VoiceSemanticResolveContext, todo: Todo): VoiceTodoReference {
...
function laneForTodo(board: Board, todo: Todo): VoiceLaneReference {
  const containingColumn = Object.entries(board.columns ?? {}).find(([, todos]) =>
    todos.some((candidate) => candidate.id === todo.id));
  const key = todo.columnKey
    ?? containingColumn?.[0]
    ?? voiceBoardLanes(board).find(
      (lane) => lane.key.toLocaleLowerCase() === todo.status.toLocaleLowerCase(),
    )?.key
    ?? todo.status;
...
function resolvedTodoCandidate(
  context: VoiceSemanticResolveContext,
  todo: Todo,
): ResolvedTodoCandidate {
  return Object.freeze({
    todo,
...
function classifyTodoCandidates(
  candidates: readonly ResolvedTodoCandidate[],
  classify: (candidate: ResolvedTodoCandidate) => VoiceSemanticCandidateDisposition,
): ClassifiedTodoCandidate[] {
...
function targetContext(context: VoiceSemanticResolveContext) {
  return {
    projectSlug: context.projectSlug,
    board: context.board,
...
async function resolveSelectedTodo(
  selection: NonNullable<VoiceSemanticSelection['todo']>,
  context: VoiceSemanticResolveContext,
): Promise<CommandResult<ResolvedTodoCandidate[]>> {
  if (!selection.allowedLocalIds.includes(selection.selectedLocalId)) {
    return localizedCommandFailure(
      'invalid_schema',
      'voice.errors.selectedTodoNotOffered',
      'Selected todo was not one of the offered choices.',
    );
  }
...
async function resolveTodoCandidates(
  target: VoiceSemanticTodoReference | VoiceTodoReference,
  state: Pick<VoiceConversationState, 'activeTodo'>,
  context: VoiceSemanticResolveContext,
  selection?: VoiceSemanticSelection['todo'],
): Promise<CommandResult<ResolvedTodoCandidate[]>> {
  if (selection) return resolveSelectedTodo(selection, context);
  ...
  if (resolved.code !== 'ambiguous_story' || !resolved.candidates?.length) return resolved;

  const candidates: ResolvedTodoCandidate[] = [];
...
async function refreshTodoCandidate(
  candidate: ResolvedTodoCandidate,
  context: VoiceSemanticResolveContext,
): Promise<CommandResult<ResolvedTodoCandidate>> {
  if (!context.callTool) return { ok: true, value: candidate };
  try {
    const response = await context.callTool<TodoGetResponse>('todos_get', {
      projectSlug: context.projectSlug,
...
async function refreshTodoCandidates(
  candidates: readonly ResolvedTodoCandidate[],
  context: VoiceSemanticResolveContext,
): Promise<CommandResult<ResolvedTodoCandidate[]>> {
  const refreshed: ResolvedTodoCandidate[] = [];
...
function projectTagNames(board: Board): string[] {
...
function matchingTagNames(reference: VoiceSemanticTagReference, board: Board): string[] {
...
function selectedTagName(
  selection: NonNullable<VoiceSemanticSelection['tag']>,
  context: VoiceSemanticResolveContext,
): CommandResult<string> {
  if (!selection.allowedNames.includes(selection.selectedName)) {
    return localizedCommandFailure(
      'invalid_schema',
      'voice.errors.selectedTagNotOffered',
      'Selected tag was not one of the offered choices.',
    );
  }
  const selected = projectTagNames(context.board).find(
    (name) => normalizeLookup(name) === normalizeLookup(selection.selectedName),
  );
  return selected
...
function todoHasTag(todo: Todo, tagName: string): boolean {
  const wanted = normalizeLookup(tagName);
  return (todo.tags ?? []).some((tag) => normalizeLookup(tag) === wanted);
}

function tagsAfterAdd(todo: Todo, tagName: string): string[] {
  return todoHasTag(todo, tagName) ? [...(todo.tags ?? [])] : [...(todo.tags ?? []), tagName];
}

function tagsAfterRemove(todo: Todo, tagName: string): string[] {
  const wanted = normalizeLookup(tagName);
  return (todo.tags ?? []).filter((tag) => normalizeLookup(tag) !== wanted);
}

function appendedNotes(body: string | undefined, notes: string): string {
  const existing = body ?? '';
...
function buildResolvedCommand(
  ir: CommandIR,
  context: VoiceSemanticResolveContext,
  details: Pick<ResolvedCommand, 'danger' | 'requiresConfirmation'>
    & Partial<Pick<ResolvedCommand, 'storyTitle' | 'statusName' | 'assigneeName'>>,
  ...
  const unresolvedDisplay: ResolvedCommand = {
    ir: validated.value,
    summary: '',
    confirmLabel: '',
...
function uniqueMembers(members: readonly BoardMember[]): BoardMember[] {
  return Array.from(new Map(members.map((member) => [member.userId, member])).values());
}

function matchingMembers(
  reference: VoiceSemanticMemberReference,
  members: readonly BoardMember[],
): BoardMember[] {
  const wanted = normalizeLookup(reference.text);
  if (!wanted) return [];
  ...
  if (reference.kind === 'email') return exactEmail;
...
async function projectMembers(
  ...
  if (!requireRefresh || !context.callTool) return context.members;
  try {
    const response = await context.callTool<MembersListResponse>('members_list', {
      projectSlug: context.projectSlug,
    });
    if (!Array.isArray(response?.items)) return context.members;
...
async function selectedMember(
  selection: NonNullable<VoiceSemanticSelection['member']>,
  context: VoiceSemanticResolveContext,
): Promise<CommandResult<BoardMember>> {
  if (!selection.allowedUserIds.includes(selection.selectedUserId)) {
    return localizedCommandFailure(
      'invalid_schema',
      'voice.errors.selectedMemberNotOffered',
      'Selected member was not one of the offered choices.',
    );
  }
...
function failureFromEmptyCandidates(): CommandFailure {
  return localizedCommandFailure(
    'unknown_story',
    'voice.errors.todoNotFound',
    'Todo was not found in this project.',
  );
}

export async function resolveVoiceSemanticIntent(
  intent: VoiceSemanticIntent,
  state: VoiceConversationState,
  context: VoiceSemanticResolveContext,
  selection: VoiceSemanticSelection = {},
  ...
  if (intent.kind === 'create-todo') {
    ...
    if (!destination) {
      return localizedCommandFailure(
        'unknown_status',
        'voice.errors.statusNotFound',
        'Status was not found on this board.',
      );
    }
  ...
  if (intent.kind === 'count-completed-todos') {
    if (!context.callTool) {
      return localizedCommandFailure(
        'network',
        'voice.errors.completedCountUnavailable',
        'The completed todo count is unavailable.',
      );
    }
    try {
      const response = await context.callTool<CompletedTodoCountResponse>('todos_countCompleted', {
        projectSlug: context.projectSlug,
      ...
      return {
        ok: true,
        value: information(
          count === 1 ? 'voice.info.completedThisWeekOne' : 'voice.info.completedThisWeek',
          count === 1
            ? '1 story was completed this week.'
            : '{count} stories were completed this week.',
          { count },
        ),
      };
    } catch {
  ...
  if (intent.kind === 'move-todo') {
    ...
    const classified = classifyTodoCandidates(candidates.value, (candidate) =>
      candidate.lane.key === lane.value.key ? 'already-satisfied' : 'actionable');
    ...
    if (actionable.length === 0) {
      const satisfied = classified.find(
        (candidate) => candidate.disposition === 'already-satisfied',
      )?.candidate;
      if (!satisfied) return failureFromEmptyCandidates();
      return {
        ok: true,
        value: information(
          'voice.info.alreadyInLane',
          '{title} is already in {lane}.',
          { title: satisfied.todo.title, lane: lane.value.name },
        ),
      };
    }
  ...
  if (intent.kind === 'assign-todo') {
    ...
    if (!resolvedMember) {
      ...
      if (members.length === 0) {
        return localizedCommandFailure(
          'unknown_user',
          'voice.errors.assigneeNotFound',
          'Assignee was not found in this project.',
        );
      }
      if (members.length > 1) {
    ...
    if (isCommandFailure(candidates)) return candidates;
    const classified = classifyTodoCandidates(candidates.value, (candidate) =>
      candidate.todo.assigneeUserId === resolvedMember.userId
        ? 'already-satisfied'
        : 'actionable');
    const actionable = classified
      .filter((candidate) => candidate.disposition === 'actionable')
    ...
    if (actionable.length === 0) {
      ...
      return {
        ok: true,
        value: information(
          'voice.info.alreadyAssigned',
          '{title} is already assigned to {member}.',
          { title: satisfied.todo.title, member: resolvedMember.name || resolvedMember.email },
        ),
      };
    }
    const target = actionable[0];
  ...
  if (intent.kind === 'add-todo-tag' || intent.kind === 'remove-todo-tag') {
    ...
    if (selection.tag) {
      ...
      if (isCommandFailure(selected)) return selected;
    ...
    if (!tagName) {
      ...
      if (matches.length === 0) {
        return localizedCommandFailure(
          'unknown_tag',
          'voice.errors.tagNotFound',
          'Tag was not found in this project.',
        );
      }
      if (matches.length > 1) {
    ...
    if (actionable.length === 0) {
      ...
      return {
        ok: true,
        value: information(
          adding ? 'voice.info.tagAlreadyPresent' : 'voice.info.tagAlreadyAbsent',
          adding
            ? '{title} already has tag {tag}.'
            : '{title} does not have tag {tag}.',
          { title: satisfied.todo.title, tag: tagName },
          satisfied.reference,
        ),
      };
    }
  ...
  if (intent.kind === 'append-todo-notes' || intent.kind === 'replace-todo-notes') {
    ...
    const classified = classifyTodoCandidates(fresh.value, (candidate) =>
      !appending && (candidate.todo.body ?? '') === intent.notes
        ? 'already-satisfied'
        : 'actionable');
    const actionable = classified
      .filter((candidate) => candidate.disposition === 'actionable')
    ...
    if (actionable.length === 0) {
      ...
      return {
        ok: true,
        value: information(
          'voice.info.notesUnchanged',
          '{title} already has those notes.',
  ...
  if (intent.kind === 'unassign-todo') {
    let namedMember: BoardMember | undefined;
    ...
    if (actionable.length === 0) {
      ...
      return {
        ok: true,
        value: information(
          namedMember ? 'voice.info.notAssignedToMember' : 'voice.info.alreadyUnassigned',
          namedMember
  ...
  if (intent.kind === 'inspect-todo') {
    ...
    if (!target) return failureFromEmptyCandidates();
    switch (intent.aspect) {
      case 'summary':
        return {
          ok: true,
          value: information(
            'voice.info.todoSummary',
            '#{localId} {title} is in {lane}.',
            { localId: target.todo.localId, title: target.todo.title, lane: target.lane.name },
            target.reference,
          ),
        };
      case 'lane':
        return {
          ...
          value: information(
            'voice.info.todoLane',
            '{title} is in {lane}.',
            { title: target.todo.title, lane: target.lane.name },
            target.reference,
          ),
        };
      case 'tags': {
        ...
        return {
          ok: true,
          value: information(
            tags ? 'voice.info.todoTags' : 'voice.info.todoTagsNone',
            tags ? '{title} has tags: {tags}.' : '{title} has no tags.',
            { title: target.todo.title, tags },
            target.reference,
          ),
        };
      }
      case 'notes': {
        ...
        return {
          ok: true,
          value: information(
            notes ? 'voice.info.todoNotes' : 'voice.info.todoNotesNone',
            notes ? 'Notes for {title}: {notes}' : '{title} has no notes.',
            { title: target.todo.title, notes },
            target.reference,
          ),
        };
      }
      case 'assignee': {
        ...
        return {
          ok: true,
          value: information(
            member ? 'voice.info.todoAssignee' : 'voice.info.todoAssigneeUnavailable',
            member
              ? '{title} is assigned to {member}.'
              : '{title} is assigned to a member who is no longer available.',
            { title: target.todo.title, member: member ?? '' },
            target.reference,
          ),
        };
      }
  ...
  if (intent.kind === 'update-todo-title') {
    ...
    if (title === null) {
      return localizedCommandFailure(
        'invalid_title',
        'voice.errors.schema.titleLength',
        'Todo title must be between 1 and 200 characters.',
      );
    }
    const classified = classifyTodoCandidates(candidates.value, (candidate) =>
      normalizeLookup(candidate.todo.title) === normalizeLookup(title)
        ? 'already-satisfied'
        : 'actionable');
    const actionable = classified
      .filter((candidate) => candidate.disposition === 'actionable')
    ...
    if (actionable.length === 0) {
      ...
      return {
        ok: true,
        value: information(
          'voice.info.titleUnchanged',
          '{title} already has that title.',
          { title: satisfied.todo.title },
        ),
      };
    }
  ...
  const target = candidates.value[0];
  if (!target) return failureFromEmptyCandidates();
  const ir: CommandIR = intent.kind === 'open-todo'
    ? {
        intent: 'open_todo',
        projectId: context.projectId,
        projectSlug: context.projectSlug,
