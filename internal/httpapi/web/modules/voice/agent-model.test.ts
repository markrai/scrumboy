...
describe('local agent provider boundary', () => {
  ...
  it('uses the local capability with v11 domain instructions and bounded output', async () => {
    const generate = vi.fn(async request => ({ requestId: request.requestId, text: '{"kind":"finish"}' }));
    ...
    expect(request.requestId).toMatch(/^voice-agent-v11-/); expect(request.maximumOutputTokens).toBe(256); expect(request.signal).toBe(signal);
