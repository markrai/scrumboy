...
export type VoiceCreateTagRepairCandidate = Readonly<{ id: string; name: string }>;
export type VoiceCreateTagRepairReference = Readonly<{
  referenceIndex: number;
  reference: string;
  candidates: readonly VoiceCreateTagRepairCandidate[];
}>;
export type VoiceCreateTagRepairRequest = Readonly<{
  transcript: string;
  references: readonly VoiceCreateTagRepairReference[];
}>;
export type VoiceCreateTagRepairBinding = Readonly<{ referenceIndex: number; tag: string }>;
export type VoiceCreateTagRepairResult = Readonly<{
  result: 'selected' | 'ambiguous' | 'invalid' | 'none';
  candidateCount: number;
  bindings: readonly VoiceCreateTagRepairBinding[];
}>;
export type VoiceCreateTagSemanticRepair = (
  request: VoiceCreateTagRepairRequest,
  signal: AbortSignal,
) => Promise<VoiceCreateTagRepairResult>;
...
export function createVoiceCreateTagRepairRequest(
  transcript: string,
  references: readonly string[],
  unresolvedReferenceIndexes: readonly number[],
  authoritativeTags: readonly VoiceCreateTag[],
): VoiceCreateTagRepairRequest | null {
  ...
  if (!candidateNamesByReference.length || candidateNamesByReference.some(group => group.candidates.length === 0)) return null;

  const candidateNames = [...new Set(candidateNamesByReference.flatMap(group => group.candidates))];
  if (!candidateNames.length || candidateNames.length > VOICE_CREATE_TAG_REPAIR_MAX_CANDIDATES) return null;
  if (new Set(candidateNames.map(name => normalizeLookup(name))).size !== candidateNames.length) return null;
...
function result(
  value: VoiceCreateTagRepairResult['result'],
  request: VoiceCreateTagRepairRequest,
  bindings: readonly VoiceCreateTagRepairBinding[] = [],
): VoiceCreateTagRepairResult {
...
export function parseVoiceCreateTagRepairResult(
  raw: string,
  request: VoiceCreateTagRepairRequest,
): VoiceCreateTagRepairResult {
  if (typeof raw !== 'string' || raw.length > 2048 || !/^\s*\{[\s\S]*\}\s*$/.test(raw)) return result('invalid', request);
  let value: unknown;
  try { value = JSON.parse(raw); } catch { return result('invalid', request); }
  if (!value || typeof value !== 'object' || Array.isArray(value)) return result('invalid', request);
  const object = value as Record<string, unknown>;
  if (Object.keys(object).some(key => key !== 'version' && key !== 'selections')
    || object.version !== 1
    || !Array.isArray(object.selections)) return result('invalid', request);
  if (object.selections.length === 0) return result('none', request);
  if (object.selections.length !== request.references.length) return result('ambiguous', request);

  const references = new Map(request.references.map(reference => [reference.referenceIndex, reference]));
  const seen = new Set<number>();
  const bindings: VoiceCreateTagRepairBinding[] = [];
  for (const item of object.selections) {
    if (!item || typeof item !== 'object' || Array.isArray(item)) return result('invalid', request);
    ...
    if (Object.keys(selection).some(key => key !== 'referenceIndex' && key !== 'tagId')
      ...
      || typeof selection.tagId !== 'string') return result('invalid', request);
    const referenceIndex = selection.referenceIndex as number;
    const reference = references.get(referenceIndex);
    if (!reference || seen.has(referenceIndex)) return result('invalid', request);
    const candidate = reference.candidates.find(value => value.id === selection.tagId);
    if (!candidate) return result('invalid', request);
...
export function createVoiceCreateTagSemanticRepair(
  capability: Pick<LocalTextGenerationCapability, 'generate'>,
): VoiceCreateTagSemanticRepair {
  return async (request, signal) => {
    if (signal.aborted) return result('invalid', request);
    ...
    try {
      ...
      if (signal.aborted || response.requestId !== requestId) return result('invalid', request);
