...
export function parseVoiceInterpretationEnvelope(raw: unknown): VoiceInterpretationResult {
  if (
    typeof raw !== 'string'
    || raw.length === 0
  ...
  const commandValue = (envelope as { command?: unknown }).command;
  if (commandValue === null) return { kind: 'refused' };
