...
export type VoiceDialogueChoiceResolution =
  | Readonly<{
      kind: 'selected';
      index: number;
  ...
  | Readonly<{ kind: 'ambiguous'; indexes: readonly number[] }>;

function namedMatches(wantedText: string, values: readonly string[]): number[] {
...
function selectedResult(
  ...
  index: number,
): VoiceDialogueChoiceResolution {
  const choice = pending.choices[index];
  if (!choice) return Object.freeze({ kind: 'no-match' });
  const selected: VoiceConversationSelection = choice.kind === 'todo'
    ? {
        todo: {
...
function finishMatches(
  ...
  if (matches.length === 0) return Object.freeze({ kind: 'no-match' });
  if (matches.length > 1) {
...
export function resolvePendingClarificationSelector(
  pending: VoicePendingClarification,
  ...
  if (!first) return Object.freeze({ kind: 'no-match' });
  if (first.kind === 'todo') {
  ...
  if (selector.kind !== 'text') return Object.freeze({ kind: 'no-match' });
...
export function pendingForTodoCorrection(
  choices: readonly VoiceTodoClarificationChoice[],
  selection: VoiceConversationSelection,
): VoicePendingClarification {
  return Object.freeze({
    kind: 'clarification',
...
export function resolveRetainedTodoSelector(
  choices: readonly VoiceTodoClarificationChoice[],
  selection: VoiceConversationSelection,
  selector: VoiceTodoChoiceSelector,
): VoiceDialogueChoiceResolution {
  return resolvePendingClarificationSelector(
    pendingForTodoCorrection(choices, selection),
    selector,
  );
}
