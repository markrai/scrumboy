...
import { VoiceAgentResourceHandles, type AgentResource } from './agent-resources.js';

export type AgentChoice = Readonly<{ handle: string; label: string; number?: number; lane?: string }>;
export type AgentSkillResult =
  | { status: 'resolved' | 'opened'; todoRef: string; title: string; number: number; lane: string }
  | { status: 'inspected'; todoRef: string; facts: readonly { field: string; value: string }[] }
  | { status: 'choices'; resource: AgentResource['kind']; choices: readonly AgentChoice[] }
  | { status: 'prepared'; proposalRef: string; summary: string }
  | { status: 'count'; count: number }
  | { status: 'not_found'; resource: AgentResource['kind'] }
  | { status: 'no_op' | 'stale' | 'denied' | 'invalid' };
export type PreparedSkill = Readonly<{ call: SkillCall; command: ResolvedCommand; fingerprint: string }>;
...
export type SkillPorts = {
  ...
  execute?: typeof executeCommandIR;
};
type SkillDefinition = { effect: 'read' | 'open' | 'proposal'; run(call: SkillCall, task: AgentSkillContext, signal: AbortSignal): Promise<SkillOutcome> };
...
function unwrapTodoReference(reference: string): string | null {
...
export class VoiceAgentSkillRegistry {
  ...
  private readonly execute: typeof executeCommandIR;
  constructor(readonly options: VoiceCommandOptions, ports: SkillPorts = {}) {
    ...
    this.skills = new Map(SKILL_NAMES.map(name => [name, Object.freeze({
      effect: name === 'todos.open' ? 'open' as const : ['todos.resolve', 'todos.inspect', 'analytics.count_completed'].includes(name) ? 'read' as const : 'proposal' as const,
      run: (call: SkillCall, task: AgentSkillContext, signal: AbortSignal) => this.resolve(call, task, signal),
    })]));
  }
  context(signal: AbortSignal): VoiceCommandContext {
    if (signal.aborted) throw new AgentProtocolError('Task cancelled');
    const result = getActiveVoiceCommandContext(this.options);
    if (isCommandFailure(result)) throw new AgentProtocolError('Context invalidated');
    return result.value;
  }
  private async tool<T>(name: McpToolName, input: Record<string, unknown>, signal: AbortSignal): Promise<T> {
    this.context(signal);
    const result = await this.callTool<T>(name, input, { signal });
    this.context(signal);
    return result;
  }
  private lane(todo: Todo, board: Board): string {
    return todo.columnKey ?? Object.entries(board.columns).find(([, values]) => values.some(t => t.id === todo.id))?.[0] ?? todo.status;
  }
  private async fresh(resource: Extract<AgentResource, { kind: 'todo' }>, signal: AbortSignal): Promise<Todo> {
    const context = this.context(signal);
    const response = await this.tool<{ todo?: Todo }>('todos_get', { projectSlug: context.projectSlug, localId: resource.localId }, signal);
    if (!response.todo || response.todo.localId !== resource.localId || response.todo.id !== resource.rowId) throw new AgentProtocolError('Stale todo');
    return response.todo;
  }
  private facts(todo: Todo, task: AgentSkillContext, board: Board): Extract<AgentSkillResult, { status: 'resolved' | 'opened' }> {
  ...
  private choice(task: AgentSkillContext, call: SkillCall, resource: AgentResource['kind'], choices: AgentChoice[]): SkillOutcome {
    const result = { status: 'choices' as const, resource, choices: choices.slice(0, AGENT_LIMITS.choices) };
    task.pendingChoice = { call, result }; task.choiceAnswered = false;
    return { result };
  }
  private async todos(call: SkillCall, task: AgentSkillContext, signal: AbortSignal): Promise<Todo[]> {
    const args = call.arguments;
    const context = this.context(signal);
    if (!('reference' in args) && !('todoRef' in args)) return [];
    if ('todoRef' in args && args.todoRef) return [await this.fresh(task.handles.get(args.todoRef, 'todo'), signal)];
    const reference = args.reference!;
    if (resourceLike(reference)) throw new AgentProtocolError('Use issued todoRef');
    ...
    let target = number
      ? { kind: 'id' as const, localId: number.value, ambiguousId: number.ambiguous, display: reference }
      : { kind: 'title' as const, phrase: reference, display: reference };
    const resolveContext = {
      projectSlug: context.projectSlug, board: context.board,
      callTool: <T>(name: McpToolName, input: Record<string, unknown>) => this.tool<T>(name, input, signal),
    };
    let resolved;
    if (target.kind === 'title' && unwrapped) {
      ...
      if (exactOriginal) resolved = exactOriginal;
      else {
        target = { kind: 'title' as const, phrase: unwrapped, display: reference };
        resolved = await resolveTodoTarget(target, resolveContext);
        if (!isCommandFailure(resolved) && !strippedReferenceIsStronger(reference, unwrapped, resolved.value.todo)) return [];
      }
    } else resolved = await resolveTodoTarget(target, resolveContext);
    this.context(signal);
    ...
    for (const candidate of candidates.slice(0, AGENT_LIMITS.choices)) {
      const response = await this.tool<{ todo?: Todo }>('todos_get', { projectSlug: context.projectSlug, localId: candidate.localId }, signal);
      if (!response.todo || response.todo.localId !== candidate.localId) throw new AgentProtocolError('Stale target');
      if (target.kind === 'title' && response.todo.title !== candidate.title) throw new AgentProtocolError('Title changed during resolution');
      values.push(response.todo);
    }
    return values;
  }
  async run(call: SkillCall, task: AgentSkillContext, signal: AbortSignal): Promise<SkillOutcome> {
    const definition = this.skills.get(call.skill);
    if (!definition) throw new AgentProtocolError('Unknown skill');
    const supplied = call.arguments;
    if ('todoRef' in supplied && supplied.todoRef) task.handles.get(supplied.todoRef, 'todo');
    if ('member' in supplied && supplied.member && resourceLike(supplied.member)) task.handles.get(supplied.member, 'member');
    if ('tag' in supplied && resourceLike(supplied.tag)) task.handles.get(supplied.tag, 'tag');
    if ('lane' in supplied && supplied.lane && resourceLike(supplied.lane)) task.handles.get(supplied.lane, 'lane');
    if (task.pendingChoice) {
      ...
      if (!task.choiceAnswered || call.skill !== pending.call.skill || !pending.result.choices.some(choice => choice.handle === selection)) throw new AgentProtocolError('Select only an offered choice after the user replies');
      // Selection may replace only the ambiguous resource; authored content and other targets stay bound.
      const clean = (value: SkillCall['arguments']) => {
        const copy = { ...value } as Record<string, unknown>;
        for (const key of pending.result.resource === 'todo' ? ['reference', 'todoRef'] : [pending.result.resource]) delete copy[key];
        return JSON.stringify(Object.keys(copy).sort().map(key => [key, copy[key]]));
      };
      if (clean(call.arguments) !== clean(pending.call.arguments)) throw new AgentProtocolError('Choice changed the operation');
      task.pendingChoice = null;
    }
    return definition.run(call, task, signal);
  }
  private async resolve(call: SkillCall, task: AgentSkillContext, signal: AbortSignal): Promise<SkillOutcome> {
    let context = this.context(signal);
    const mutation = this.skills.get(call.skill)!.effect === 'proposal';
    if (mutation && !canRunVoiceMutationInContext(context)) return fail('denied');
    const args = call.arguments;
    if (call.skill === 'analytics.count_completed') {
      const response = await this.tool<{ count?: number }>('todos_countCompleted', { projectSlug: context.projectSlug, period: 'this-week', timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC' }, signal);
      if (!Number.isSafeInteger(response.count) || response.count! < 0) return fail('invalid');
      return { result: { status: 'count', count: response.count! } };
    }
    let lane: ReturnType<typeof voiceBoardLanes>[number] | undefined;
    let member: BoardMember | undefined;
    let tag: string | undefined;
    let boundCall: SkillCall = call;
    if ('lane' in args && args.lane !== undefined) {
      if (resourceLike(args.lane)) {
        ...
        if (isCommandFailure(resolved)) {
          if (resolved.code !== 'ambiguous_status') return fail('not_found', 'lane');
      ...
      if (!lane) return fail('stale');
      boundCall = { ...call, arguments: { ...args, lane: task.handles.issue({ kind: 'lane', key: lane.key }) } } as SkillCall;
    }
    if (call.skill === 'todos.create' && !lane) {
      lane = voiceBoardLanes(context.board)[0];
      if (!lane) return fail('not_found', 'lane');
    ...
    if ('member' in args && args.member !== undefined) {
      ...
      member = matches[0];
      boundCall = { ...boundCall, arguments: { ...boundCall.arguments, member: task.handles.issue({ kind: 'member', userId: member.userId }) } } as SkillCall;
    }
    if ('tag' in args) {
      ...
      tag = matches[0];
      boundCall = { ...boundCall, arguments: { ...boundCall.arguments, tag: task.handles.issue({ kind: 'tag', name: tag }) } } as SkillCall;
    }
    const candidates = await this.todos(call, task, signal);
    context = this.context(signal);
    const satisfied = (todo: Todo): boolean => {
      switch (call.skill) {
    ...
    if (call.skill !== 'todos.create' && !candidates.length) return fail('not_found');
    if (candidates.length && !actionable.length) return { result: { status: 'no_op' } };
    if (actionable.length > 1) return this.choice(task, call, 'todo', actionable.map(todo => {
    ...
    if (todo) {
      const { reference: _reference, ...rest } = boundCall.arguments as { reference?: string };
      boundCall = { ...boundCall, arguments: { ...rest, todoRef: facts!.todoRef } } as SkillCall;
    }
    ...
    let ir: CommandIR;
    switch (call.skill) {
      case 'todos.create': ir = { ...base, intent: 'todos.create', entities: { title: call.arguments.title, columnKey: lane!.key } }; break;
      case 'todos.move': ir = { ...base, intent: 'todos.move', entities: { localId: todo.localId, toColumnKey: lane!.key } }; break;
      case 'todos.rename': ir = { ...base, intent: 'todos.update_title', entities: { localId: todo.localId, title: call.arguments.title } }; break;
      case 'todos.assign': ir = { ...base, intent: 'todos.assign', entities: { localId: todo.localId, assigneeUserId: member!.userId } }; break;
      case 'todos.unassign': ir = { ...base, intent: 'todos.unassign', entities: { localId: todo.localId, assigneeUserId: null } }; break;
      case 'todos.delete': ir = { ...base, intent: 'todos.delete', entities: { localId: todo.localId } }; break;
      case 'todos.append_notes': case 'todos.replace_notes': {
        const notes = call.arguments.text;
        const old = todo.body ?? '';
        const body = call.skill === 'todos.replace_notes' || !old ? notes : `${old}${old.endsWith('\n') ? '' : '\n'}${notes}`;
        ir = { ...base, intent: call.skill, entities: { localId: todo.localId, notes, body } }; break;
      }
      case 'todos.add_tag': case 'todos.remove_tag': ir = { ...base, intent: call.skill, entities: { localId: todo.localId, tag: tag!, tags: call.skill === 'todos.add_tag' ? [...(todo.tags ?? []), tag!] : (todo.tags ?? []).filter(name => normalizeLookup(name) !== normalizeLookup(tag!)) } }; break;
  ...
  async commit(prepared: PreparedSkill, signal: AbortSignal): Promise<void> {
    const context = this.context(signal);
    if (!canRunVoiceMutationInContext(context)) throw new AgentProtocolError('Permission denied');
...
export function renderAgentSkillResult(result: AgentSkillResult): string {
  switch (result.status) {
    ...
    case 'opened': case 'resolved': return `#${result.number} · ${result.title} · ${result.lane}`;
