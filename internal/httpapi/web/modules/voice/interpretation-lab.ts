...
import {
  ...
  type RunInterpretationLabBatchOptions,
  type VoiceInterpretationDiagnostic,
...
type InterpretationLabCapabilityRegistry = {
  capability(name: typeof LOCAL_TEXT_GENERATION_CAPABILITY): LocalTextGenerationCapability | null;
};

export type MountInterpretationLabOptions = {
  getResolveContext: () => InterpretationLabResolveContext | null;
  runtime?: InterpretationLabCapabilityRegistry;
  getLocale?: () => string;
  runBatch?: (options: RunInterpretationLabBatchOptions) => Promise<VoiceInterpretationDiagnostic[]>;
  writeClipboard?: (text: string) => Promise<void>;
  now?: () => Date;
};

export type MountedInterpretationLab = {
  dispose(): void;
};

export function createInterpretationLabCopyPayload(
  results: readonly VoiceInterpretationDiagnostic[],
  timestamp: string,
): object {
  return {
...
function readinessMessage(status: LocalTextGenerationStatus): string {
  switch (status.state) {
    case 'unsupported':
...
function profileLabel(profile: InterpretationLabProfile): string {
  switch (profile) {
...
function stageLabel(stage: { state: string }): string {
  return stage.state;
}
...
function renderLabResults(container: HTMLElement, results: readonly VoiceInterpretationDiagnostic[]): void {
...
export function mountInterpretationLab(
  dialog: HTMLDialogElement,
  options: MountInterpretationLabOptions,
): MountedInterpretationLab {
  const anchor = dialog.querySelector<HTMLElement>('.voice-command__review');
  ...
  const toggle = launcher.querySelector<HTMLButtonElement>('#voiceInterpretationLabToggle');
  const corpus = panel.querySelector<HTMLTextAreaElement>('#voiceInterpretationLabCorpus');
  const runCurrent = panel.querySelector<HTMLButtonElement>('#voiceInterpretationLabRunCurrent');
  const runExperimental = panel.querySelector<HTMLButtonElement>('#voiceInterpretationLabRunExperimental');
  const runCandidate = panel.querySelector<HTMLButtonElement>('#voiceInterpretationLabRunCandidate');
  const runAll = panel.querySelector<HTMLButtonElement>('#voiceInterpretationLabRunAll');
  const stop = panel.querySelector<HTMLButtonElement>('#voiceInterpretationLabStop');
  const copy = panel.querySelector<HTMLButtonElement>('#voiceInterpretationLabCopy');
  const clear = panel.querySelector<HTMLButtonElement>('#voiceInterpretationLabClear');
  const status = panel.querySelector<HTMLElement>('#voiceInterpretationLabStatus');
  const resultContainer = panel.querySelector<HTMLElement>('#voiceInterpretationLabResults');
  if (corpus) corpus.value = INTERPRETATION_LAB_SEED_CORPUS;
  ...
  const setStatus = (text: string) => {
    if (!disposed && status) status.textContent = text;
  };
  ...
  const setRunning = (running: boolean) => {
    ...
    if (copy) copy.disabled = running;
    if (corpus) corpus.readOnly = running;
  };

  const renderResults = () => {
    if (!disposed && resultContainer) renderLabResults(resultContainer, results);
  };

  const runProfiles = async (profiles: readonly InterpretationLabProfile[]) => {
    if (disposed || runningController) return;
    ...
    try {
      ...
      if (disposed || runningController !== controller) return;
  ...
  copy?.addEventListener('click', () => {
    ...
    void writeClipboard(JSON.stringify(payload, null, 2)).then(
      () => setStatus('Results copied.'),
      () => setStatus('Clipboard unavailable. Results remain in memory.'),
    );
  });
