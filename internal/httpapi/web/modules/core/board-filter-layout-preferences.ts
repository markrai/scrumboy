...
export type BoardFilterLayout = 'omni' | 'legacy';

export const BOARD_FILTER_LAYOUT_DEFAULT: BoardFilterLayout = 'omni';
...
export function normalizeBoardFilterLayout(value: unknown): BoardFilterLayout {
  return value === 'legacy' ? 'legacy' : BOARD_FILTER_LAYOUT_DEFAULT;
}

export function getBoardFilterLayoutPreference(): BoardFilterLayout {
  try {
...
export function setBoardFilterLayoutPreference(
  ...
  if (previous !== next) emit(BOARD_FILTER_LAYOUT_CHANGED_EVENT, next);
...
export function hydrateBoardFilterLayoutFromServer(value: unknown): void {
  setBoardFilterLayoutPreference(normalizeBoardFilterLayout(value), { skipRemote: true });
}

export async function loadBoardFilterLayoutPreferenceFromServer(
